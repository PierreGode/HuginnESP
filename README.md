...
|---|---|---|
| WiFi (`wardrive_wifi_ms`) | 8000 ms | A weighted per-channel sweep, capped at this value. High-traffic channels (2.4 GHz 1/6/11, plus the 5 GHz channels on the C5) are visited first and repeated, then the rest are swept once. **2.4 GHz channels are scanned actively** (probe, up to ~200 ms each); **5 GHz channels are scanned passively** (listen for a beacon, up to ~300 ms each) — passive is mandatory on DFS channels (52–144) and is what makes 5 GHz APs actually appear. The C5 also enables 802.11d at init (world code `01` baseline, `ieee80211d_enabled=true`) so the radio adapts its regulatory domain to wherever it's powered on — unlocking that region's channels automatically (EU 2.4 GHz 12/13 + 5 GHz 36–140, US UNII-3 149–165) with no per-region build. The S3's 2.4 GHz list finishes well inside the cap (~3–4 s); the C5's longer dual-band list can use the full window. BSSIDs already emitted this session are de-duplicated on-device, so the host sees each AP once |
