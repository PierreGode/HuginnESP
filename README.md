...
| **802.15.4 Scan (Zigbee / Thread)** | **ESP32-C5 only.** Promiscuous IEEE 802.15.4 sniff across channels 11–26; every frame with a source address is emitted (PAN ID, EUI-64 / short address, channel, RSSI, LQI) with a `proto` tag classifying it as `zigbee` / `thread` / `802.15.4`. Matter-over-Thread reports as `thread`. Runs **on demand via the `zigbee` command** (parks WiFi/BLE so 802.15.4 gets the radio exclusively). It is **not** part of the wardrive cycle: WiFi, BLE and 802.15.4 share one 2.4 GHz radio on the C5, and once 802.15.4 receives, the WiFi scan can't reclaim the radio without a WiFi re-init that exhausts the ~116 KB heap — so wardrive stays WiFi + BLE. Not available on the S3 / classic ESP32 (no 802.15.4 radio) |
